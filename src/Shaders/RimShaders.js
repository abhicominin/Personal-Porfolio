...
/*glsl*/`
    ...
    void main()
    {
        ...
        vec2 radialDirection = normalize(_worldPos); // Normalize to get direction
        float displacement = sin(_Time + length(_worldPos) * 2.0) * 2.1; // Radial displacement over time

        // Apply radial displacement
        worldPos.xz += radialDirection * displacement;
...
/*glsl*/`
    ...
    void main()
    {
        // Time-based factor for easing motion
        float timeFactor = sin(_Time * 0.5) * 0.5 + 0.5; // Oscillates between 0.0 and 1.0

        // Define the two colors for transition
        vec3 colorA = vec3(0.0, 0.5, 1.0); // Purple
        vec3 colorB = vec3(1.0, 0.5, 0.0); // Orange

        // Interpolate between the colors based on timeFactor
        vec3 tintColor = mix(colorA, colorB, timeFactor);

        // Simulating the rest of the shader logic (simplified)
        vec3 viewVec = vec3(_worldPos.x, 0.0, _worldPos.y) - cameraPosition;
        float viewLen = length(viewVec);
        vec3 viewDir = viewVec / viewLen;

        vec3 normal = texture2D(_NormalMap1, _uv + VELOCITY_1 * (_Time * 10.0)).xyz * 2.0 - 1.0;
        normal += texture2D(_NormalMap2, _uv + VELOCITY_2 * (_Time * 10.0)).xyz * 2.0 - 1.0;
        normal *= NORMAL_MAP_STRENGTH * 100000.0 ;
        ...
        if (cameraPosition.y > 0.0)
        {
            ...
            // Apply the tint color to the final surface color
            surface = surface * tintColor;
        ...
        if (dot(viewDir, normal) < CRITICAL_ANGLE)
        {
            ...
            // Apply the tint color to the reflected color
            rColor = rColor * tintColor;

            gl_FragColor = vec4(mix(rColor, light, t), 1.0);
            return;
        }

        // Apply the tint color to the underwater light color
        light = light * tintColor;
...
/*glsl*/`
    ...
    void main()
    {
        ...
        float sampleY = originY + viewDir.y * viewLen;
        vec3 light = exp((sampleY - viewLen * (DENSITY * 1.5)) * (ABSORPTION * 1.1 ));
        light *= _Light;
...
export const rimobjectFragment =
/*glsl*/`
    #include <ocean>
